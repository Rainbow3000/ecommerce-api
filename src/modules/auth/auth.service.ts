...
export class AuthService {
  ...
  async getOtp(email: string) {
    const otp = this.generateRandomFiveDigitNumber();
    this.globalService.set('otp', otp);
    const token = Math.floor(1000 + Math.random() * 9000).toString();
    await this.mailService.sendOTP(email, token, otp);

    return otp;
  }

  async confirmOtp(otp: number) {
    const originOtp = this.globalService.get('otp');
  ...
  async checkEmailExisted(email: string) {
    ...
    if (user) throw new BadRequestException(USER_EXISTED);
